...
sidebar_position: 5
---
![](/img/RouteList/WaylineExport/image-0.png)  
![](/img/RouteList/WaylineExport/image-1.png)  
在航线列表点击待导出航线的 ![](/img/RouteList/WaylineExport/image-2.png) 按钮可将航线导出至本地，导出内容有航线kmz文件、航线测区kml、未插值飞行轨迹kml

## 航线拆分
![](/img/RouteList/WaylineExport/image-3.png)  
启用航线拆分时，程序会导出多个航线kmz文件，并通过一系列算法确保各个子航线的飞行姿态、录像任务等与原始单航线保持一致

## 导出航线至移动设备
![](/img/RouteList/WaylineExport/image-4.png)  
在航线导出面板设置好分割参数后，点击此处，可跳转至航线快捷导出至移动设备面板

### 普通移动设备（未安装DJI Fly / DJI Pilot）
![](/img/RouteList/WaylineExport/image-5.png)  
对于普通设备，软件可自动将航线 / 航线切片导出至 内部存储 或 内部存储/Download
软件导出无法生效时，也可通过  
![](/img/RouteList/WaylineExport/image-6.png)  
获取原始kmz文件，手动复制转移
...
![](/img/RouteList/WaylineExport/image-7.png)  
将 带屏遥控器 / 安装了DJI Fly的安卓设备 通过USB连接电脑，选择文件传输模式，软件将列出DJI Fly现有的航线，选择待替换的航线，点击导出，软件将自动执行缩略图替换、航线文件替换

#### 手动替换
当设备无法识别，或设备为ios时，需要手动替换实现航线导入
![](/img/RouteList/WaylineExport/image-6.png)  
通过此处获取航线kmz文件，将设备通过USB与电脑连接，通过资源管理器，前往 DJI Fly 航线存储路径，进行航线替换，如安卓设备：
![](/img/RouteList/WaylineExport/image-8.png)  
